...
void DataBaseType::initialize(DataBaseType* input)
{
    ...
    if (this->condition!=NULL) free(this->condition);
...
int DataBaseType::compare(char*source,int sourcelength,char*dest,int destlength)
{
    ...
    if (dest[destlength]==IS_NULL) return 1;
    for (int i=0;i<sourcelength;i++)
    {
        if (source[i]>dest[i]) return 1;
        else if (source[i]<dest[i]) return -1;
    }
    return 0;
}
